...
public sealed class MealsRecipesJsonGenerator
{
    public static void PublishJson(StreamWriter file)
    {
        ...
        var constructionGraphs =
            constructable
                .Where(x => (Regex.Match(x.ID.ToLower().Trim(), @".*.*[Bb]acon*|.*[Ss]teak*|[Pp]izza*|[Tt]ortilla*|[Ee]gg*").Success)) // we only need recipes that has "bacon", "steak" and "pizza" in it, since they are the only "constructable" recipes
                .ToDictionary(x => x.ID, x => x);
