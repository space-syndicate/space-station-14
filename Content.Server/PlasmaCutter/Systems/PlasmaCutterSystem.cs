...
public sealed class PlasmaCutterSystem : EntitySystem
{
    ...
    private void TurnOff(PlasmaCutterComponent comp)
    {
        ...
        SoundSystem.Play(comp.workSound.GetSound(), Filter.Pvs(comp.Owner), comp.Owner, AudioHelpers.WithVariation(0.25f)); //playing success sound

        comp.Activated = false; //changing activated to false
    }
    ...
    private void TurnOn(PlasmaCutterComponent comp, EntityUid user)
    {
        ...
        SoundSystem.Play(comp.workSound.GetSound(), playerFilter, comp.Owner, AudioHelpers.WithVariation(0.25f)); //playing success sound

        comp.Activated = true; //changing activated to true
    }
    ...
    private async void OnAfterInteract(EntityUid uid, PlasmaCutterComponent pc, AfterInteractEvent args)
    {
        ...
        var doAfterEventArgs = new DoAfterEventArgs(user, pc.Delay, pc.CancelToken.Token, args.Target)
        {
            BreakOnDamage = true,
            BreakOnStun = true,
            NeedHand = true,
    ...
    private bool IsPCStillValid(PlasmaCutterComponent pc, AfterInteractEvent eventArgs, MapGridComponent mapGrid, TileRef tile)
    {
        ...
        if (!pc.Activated)
        {
            _popup.PopupEntity(Loc.GetString("pc-component-not-lit"), pc.Owner, Filter.Entities(eventArgs.User));
            return false;
        }

        if (pc.CurrentAmmo <= 0)
        {
            _popup.PopupEntity(Loc.GetString("pc-component-no-ammo-message"), pc.Owner, Filter.Entities(eventArgs.User));
            return false;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("pc-component-tile-obstructed-message"), pc.Owner, Filter.Entities(eventArgs.User));
            return false;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("pc-component-deconstruct-target-not-on-whitelist-message"), pc.Owner, Filter.Entities(eventArgs.User));
            return false;
        }
        return true;
    }
