...
cmd-gridtc-desc = Получает количество плиток в сетке.
...
cmd-uitest-desc = Откройте dummy UI в окне тестирования
...
cmd-rmgrid-desc = Удаляет сетку с карты. Вы не можете удалить сетку по умолчанию.
...
cmd-lsgrid-desc = Перечисляет сетки.
...
cmd-lightbb-desc = Переключает, показывать ли световые ограничительные рамки.
